...
from insert_states import insert_game_state
...
food = pygame.image.load("snake\Food.png").convert_alpha()
...
class Snake:
    def __init__(self):
        ...
        self.snake_head_down  = pygame.image.load("snake\\SnakeHeadDown.png").convert_alpha()
        self.snake_head_down  = pygame.transform.scale(self.snake_head_down, (BLOCK_SIZE, BLOCK_SIZE))

        self.snake_head_up    = pygame.image.load("snake\\SnakeHeadUp.png").convert_alpha()
        self.snake_head_up    = pygame.transform.scale(self.snake_head_up, (BLOCK_SIZE, BLOCK_SIZE))

        self.snake_head_right = pygame.image.load("snake\\SnakeHeadRight.png").convert_alpha()
        self.snake_head_right = pygame.transform.scale(self.snake_head_right, (BLOCK_SIZE, BLOCK_SIZE))

        self.snake_head_left  = pygame.image.load("snake\\SnakeHeadLeft.png").convert_alpha()
    ...
    def play_sound(self):
        pygame.mixer.Sound("snake\\yeahoo.mp3").play()
...
class Main:
    ...
    def game_over(self):
        ...
        update_user(user_score, user)
        #update_game_state((pause, end, win, LEVEL, user))
    ...
    def win(self):
        ...
        pygame.mixer.Sound("snake\\WinMarioSound.wav").play(0)
...
EXIST = False
if len(user_names) == 0: print("new user added")
else:
    for i in user_names:
        if user == i:
            EXIST = True
...
main = Main()
user_score = len(main.snake.body) - 3
if not EXIST:
    insert_user((user, user_score))
    #insert_game_state((pause, end, win, LEVEL))
